...
#include "WavePropagationBlock1D.h"

Blocks::WavePropagationBlock1D::WavePropagationBlock1D(RealType* h, RealType* hu, RealType* b, unsigned int size, RealType cellSize):
  h_(h),
  hu_(hu),
  b_(b),
  size_(size),
...
Blocks::WavePropagationBlock1D::~WavePropagationBlock1D() {
...
RealType Blocks::WavePropagationBlock1D::computeNumericalFluxes() {
...
void Blocks::WavePropagationBlock1D::updateUnknowns(RealType dt) {
...
//Outflow boundary conditions on the left and right side
void Blocks::WavePropagationBlock1D::setOutflowBoundaryConditions() {
  h_[0]         = h_[1];
  h_[size_ + 1] = h_[size_];
